            ...
            if ($objPHPExcelAssistantes->getActiveSheet()->getCell($cell)->getColumn() === 'A' && $objPHPExcelAssistantes->getActiveSheet()->getCell($cell)->getRow() > 1) {
                ...
                if (!empty($userDB)) {
                    ...
                    if (trim($objWorksheet->getCell('F' . $rowIndex)) !== "-" && trim($objWorksheet->getCell('F' . $rowIndex)) !== '') {
                        $newUser->setAA(trim($objWorksheet->getCell('F' . $rowIndex))); // On attribut la valeur de la case d'assistante agence comme assistante d'agence.
                    }
                    ...
                    elseif (trim($objWorksheet->getCell('G' . $rowIndex)) !== "-" && trim($objWorksheet->getCell('G' . $rowIndex)) !== '') {
                        $newUser->setAA(trim($objWorksheet->getCell('G' . $rowIndex))); // On attribut la valeur de la case de directeur d'agence comme assistante d'agence.
                    }
                    ...
                    elseif (trim($objWorksheet->getCell('H' . $rowIndex)) !== "-" && trim($objWorksheet->getCell('H' . $rowIndex)) !== '') {
                        $newUser->setAA(trim($objWorksheet->getCell('H' . $rowIndex))); // On attribut la valeur de la case d'assistante RH comme assistante d'agence.
                    }
                    // Sinon.
                    else {
                        $newUser->setAA(trim($objWorksheet->getCell('I' . $rowIndex))); // On attribut la valeur de la case de N+2 comme assistante d'agence.
                    }
                    ...
                    if (trim($objWorksheet->getCell('G' . $rowIndex)) !== "-" && trim($objWorksheet->getCell('G' . $rowIndex)) !== '') {
                        $newUser->setDA(trim($objWorksheet->getCell('G' . $rowIndex))); // On attribut la valeur de la case de directeur d'agence comme directeur d'agence.
                    }
                    // Sinon la case Assistante RH n'est pas nul.
                        ...
                        $newUser->setDA(trim($objWorksheet->getCell('H' . $rowIndex))); // On attribut la valeur de la case de d'assistante RH comme directeur d'agence.
                    }
                    // Sinon.
                    else {
                        $newUser->setDA(trim($objWorksheet->getCell('I' . $rowIndex))); // On attribut la valeur de la case de N+2 comme directeur d'agence.
                    }

                    // On attribut la valeur de la case d'assistante RH comme assistante RH.
                    $newUser->setRH(trim($objWorksheet->getCell('H' . $rowIndex)));

                    // On attribut la valeur de la case de N+2 comme N+2.
                    $newUser->setN2(trim($objWorksheet->getCell('I' . $rowIndex)));

                    // On attribut la valeur de la case de Matricule comme matricule.
                    $newUser->setMatricule(trim($objWorksheet->getCell('A' . $rowIndex)));

                    // On attribut l'agence.
                    $newUser->setEtablissement(trim($objWorksheet->getCell('C' . $rowIndex)));

                    // On attribut l'agence.
                    $newUser->setSociete(trim($objWorksheet->getCell('B' . $rowIndex)));
