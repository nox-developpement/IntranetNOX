...
class PointageAjaxController extends Controller {
    ...
    public function generateExcelRecapAction(Request $request) {
        if ($request->isXmlHttpRequest()) {
            ...
            foreach ($pointagesValides as $pointage) {
                ...
                $filename = str_replace('/', '_', 'Récapitulatif compilation pointages - ' . $securityName . ' - ' . $monthString[$month] . ' ' . $year . '.xlsx'); // On génére le nom de fichier.
    ...
    public function ajaxGetPointageListAction(Request $request) {
        if ($request->isXmlHttpRequest()) {
            ...
            $em = $this->getDoctrine()->getManager();
            $userHierarchy = $em->getRepository('NoxIntranetPointageBundle:UsersHierarchy')->findBy(array('etablissement' => $etablissement));
    ...
    public function ajaxSaveNOXIPCSVAction(Request $request) {
        if ($request->isXmlHttpRequest()) {
            ...
            // Si le tableau existe et le collaborateur fait partie de NOX IP...
            if (!empty($userHierarchy) && !empty($tableau) && strpos($userHierarchy->getEtablissement(), 'INDUSTRIE') !== false && strpos($userHierarchy->getEtablissement(), 'PROCESS') !== false) {
                // On convertie les données JSON en Array.
