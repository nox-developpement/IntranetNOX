...
//    });
    ...
    $('#divAffichageImages img').click(function () {
        if ($(this).css('max-height') !== 'none') {
            ...
            $(this).css('cursor', "url('http://" + $(location).attr('host') + "/Symfony/web/images/Minus-32.png'), auto");
        } else {
            $(this).removeAttr('style');
            $(this).css('max-height', $(window).height() / 5);
            $(this).css('cursor', "url('http://" + $(location).attr('host') + "/Symfony/web/images/Plus-32.png'), auto");
        }
    });
});
